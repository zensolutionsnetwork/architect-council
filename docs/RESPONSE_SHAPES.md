...
and is excluded until it uploads one. `GET /api/council/brains` now lists the full seating roster. **#54 lifecycle
(2026-07-06, the REVERSE of register): `POST /api/council/agents/:id/retire` → `{ok, id, status:"retired", seats:[...]}`
drops the seat from `council_seats`, marks the member row `active=false` (KEPT, auditable) and revokes its secret;
`DELETE /api/council/agents/:id` → `{ok, id, status:"deleted", seats:[...]}` drops the seat AND hard-deletes the
member row + secret (cascade). Both are owner-gated; a founding seat → 409 `founding_seat`; an id not in the
app-registered roster → 404 `unknown_agent`; a bad id → 400 `bad_id`. Same roster-separation INVARIANT in reverse:
neither can touch a `MEETING_DEFAULT` member, so removing an extra 5th seat can never flip an adopted standard to
partial. `seats` = the recomputed seating roster.** Prior:_
...
- **`POST /api/council/commitments`** (owner/hub only) `{ sourceMeetingId?, title, detail?, proposedBy?, forActors:[...], sourceAgendaId? }` -> `{ ok, created:[commitment...] }` (one row per addressed actor, deduped). `400 forActors_required` / `title is required`.
  - **Finalizer auto-mint (#67, 2026-07-09):** at every real (non-dry-run) meeting close the owner-report synthesis call ALSO emits a trailing `perAgentCommitments` JSON block; the finalizer parses it (stripped from the stored report, so the `owner-report` shape is UNCHANGED) and mints `proposed` rows with `proposedBy:"hub"` — one per addressed agent. Reuses the SAME close-time model call (no new spend); runs only on the first close (idempotent on `closed_at`, so a re-close never double-mints); best-effort (a mint failure is logged and never fails the close).
