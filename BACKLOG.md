...
- **P1 - [#56]** Hub `zut1.*` upload-token `exp` clamp (60s..7d) + `ns` allow-list - server-enforcement of the 07-03 round #41/#42. **BLOCKED on Arke contract (07-04 day session):** the hub src+docs have NO `zut1`/`X-Upload-Token`/upload-token/cockpit-publish surface at all, so before shipping I asked Arke (msg `5a59c869`, high) to pin: (a) HUB mints (cockpit calls a hub endpoint with `{ns,expires_in}`, hub clamps+allow-lists+issues) vs cockpit self-mints client-side and the hub VERIFIES on an upload endpoint; (b) which endpoint carries `X-Upload-Token` + what it protects ON THE HUB (or is the publish target Arke's app, not the hub -> #56 is app-side); (c) token format (HMAC shared-secret vs hub-issued opaque). Ship the clamp + `ns=council-cockpit` allow-list once (a)-(c) land. Contract to match: `ns=council-cockpit`, `expires_in` 600s default (60..604800), `X-Upload-Token` header only.
