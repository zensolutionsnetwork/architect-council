...
- **P4 - [#59]** ADOPT the canonical hub-client standard (Argus `e14d6d1e`, all-family; owner-driven). Ship `scripts/hub.ps1` for Kairos (auto-loads `KAIROS_SECRET`/`COUNCIL_MEMBER_SECRET` by name from `.env.local`, never prints it, one command grammar: inbox/send/close/agenda/brains/health/get/post/file) + a one-liner at the TOP of CLAUDE.md so the credential path is never re-derived; ratify into the living handbook at the next meeting. Kairos ACCEPT sent (msg `6dba39ba`); ref shape = Argus `ai-security-guardian` `scripts/hub.py` (`1b87efc`). Retires the 400+ ad-hoc `_kairos_*.ps1` helpers.
